...
std::wstring Internet::DownloadUrl(const std::wstring& url, int codepage)
{
	...
	BYTE* lpOutPut;							//loop输出
	BYTE* lpHolding = nullptr;
	int nCounter = 1;
	int nBufferSize;
	DWORD dwDataSize = 0;					//已经下载的大小
	DWORD dwSize = 0;						//本次下载的大小

	do
	{
		// Read the data.
		if (!InternetReadFile(hUrlDump, (LPVOID)lpData, CHUNK_SIZE, &dwSize))	
		{			//根据句柄hUrlDump，向指针lpData的位置写入不超过CHUNK_SIZE的数据，并返回实际写入的数据量dwSize
			break;
		}
		else
		{
			...
			nBufferSize = dwDataSize + dwSize;			//Buffer大小 = 已经下载的大小 + 本次下载的大小

			// Allocate the output buffer.
			lpOutPut = new BYTE[nBufferSize + 2];		//为指针lpOutPut分配Buffer大小的内存

			// Make sure the buffer is not the initial buffer.
			if (lpHolding != nullptr)			//当这不是第一次循环时
			{
				// Copy the data in the holding buffer.	  将lpHolding指向的dwDataSize大小的数据（上次下载的数据）复制到指针lpOutPut指向的位置
				memcpy(lpOutPut, lpHolding, dwDataSize);

				// Delete the old buffer
				delete [] lpHolding;					//删除上次下载的数据

				lpHolding = lpOutPut;					//lpHolding指向已下载的数据的起始位置
				lpOutPut = lpOutPut + dwDataSize;		//指针lpOutPut移动dwDataSize位，指向新数据的写入位置
			}
			else
			{
				lpHolding = lpOutPut;					//lpHolding指向开始位置
			}

			// Copy the data buffer.
			memcpy(lpOutPut, lpData, dwSize);			//将lpData的dwSize大小的数据（本次下载的数据）复制到lpOutPut

			dwDataSize += dwSize;						//更新“已下载”的数据大小的值
			...
			// Clear the buffer							//清空数据，但仍保留内存分配
...
void Internet::FillCharacterEntityReferences()
{
	struct CER
	{
		WCHAR* name;
...
/*
** Convert wide string to multibyte string. (unicode -> utf8)
** From StringUtil.cpp
*/
std::string Internet::Narrow(const WCHAR* str, int strLen, int cp)
{
